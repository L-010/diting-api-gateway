...
以下命令在 Ubuntu 服务器上执行。服务器地址为 `10.2.4.46`。
...
- 前端访问地址：`http://10.2.4.46:3000`，或浏览器实际访问的 HTTP Origin。
- 公开网关地址：`http://10.2.4.46:8000`。
...
从浏览器访问 `http://10.2.4.46:3000`。如果服务器启用了 UFW，只允许可信内网访问这两个临时 HTTP 端口，不要对公网开放。生产环境故意关闭 `/docs`、`/redoc` 和 `/openapi.json`，返回 404 属于正常行为。
